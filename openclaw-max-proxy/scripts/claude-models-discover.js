...
function speculativeCandidates(known) {
  ...
  for (const [family, version] of newestByFamily) {
    ...
    // bump 0 regenerates the family's current newest id, so the catalog can
    // rebuild itself from an empty cache. Without it, a model that exists only
    // because forward-probing found it (claude-opus-5 is not in the CLI bundle)
    // is unreachable once it stops being "ahead" of anything.
...
async function discover(opts) {
  ...
  // Always re-probe what the previous catalog held. Those are the best
  // candidates there are, and without them a full run silently drops any model
  // that is neither in the CLI bundle nor regenerated by forward-probing —
  // which is exactly the newest release in a family (claude-opus-5 was mined
  // from nowhere and speculation only generates versions *ahead* of it).
  // It also makes sticky removal meaningful: an id can only accumulate misses
  // if it actually gets probed.
  const pool0 = new Set([...cachedIds, ...speculative, ...extras]);
